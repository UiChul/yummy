...
class MainPublish(QWidget):
    ...
    def _get_versions_data(self):
        ...
        if project_id:
            ...
            for version in versions:
                ...
                print(ver_datas)
                print("ver_datas를 다 가져왔습니다.")

                # shot_code = self.shot_code
                # team_name = self.team_name
                # # version_number = version_pattern
                # got_data = shot_code + "_" + team_name + "_" + version_number
                # if got_data == code :
                #     return version
    ...
    def _get_nk_validation_info(self):
        ...
        root = nuke.root()
        nk_path = root["name"].value()                     # file path
        nk_extension = nk_path.split(".")[-1]                 # extension
        nk_colorspace = root["colorManagement"].value()    # colorspace
        nuke_version = nuke.NUKE_VERSION_STRING         # nk version
        
        nk_file_validation_dict = {
        # "file_path" : nk_path,
        "extension" : nk_extension,
        "colorspace" : nk_colorspace,
        "nuke_ver" : nuke_version
        }
        print(nk_file_validation_dict)
        print("nk_file_val_dict를 다 가져왔습니다.")
    ...
    def val_nk(self): 
        ...
        # if ver.keys == nk.keys : 
        for k in nk.keys() : 
            if not ver.get(k) == nk.get(k):                #file path도 같을 수 있나?
...
#===============================================================  
    def sg_upload_data_ver_nk(self): 
        ...
        user_id = self.sg.find_one('HumanUser', [['name', 'is', user_name]], ['id'])
    ...
    def sg_thumbnail_upload_nk(self, sg):
    ...
    # def sg_thumbnail_upload_exr(self, sg):
    ...
    # def sg_thumbnail_upload_mov(self, sg):
