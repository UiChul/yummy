...
class DraggableWidget(QWidget):
    ...
    def mousePressEvent(self, event):
        if event.buttons() & Qt.LeftButton:
            ...
            # Create a QDrag object and set the MIME data
            drag = QDrag(self)
            mime_data = QMimeData()
    ...
    def open_mov_in_rv(self):
        ...
        # RV 실행 파일의 경로 (RV가 PATH에 추가되어 있어야 함)
        rv_path = "/home/rapa/RV/bin/rv"  # 또는 "/path/to/rv"와 같이 RV의 절대 경로를 사용할 수 있음
        mov_file_path = self.file_path  # 위젯의 파일 경로

        # RV를 실행하고 MOV 파일을 인수로 전달하여 파일 자동 재생
        arguments = [mov_file_path,
        '-play',
        '-gamma','2.2'
        ]       # 자동 재생 옵션
