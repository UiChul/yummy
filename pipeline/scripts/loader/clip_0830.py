from PySide6.QtWidgets import QApplication, QTableWidget, QLabel, QVBoxLayout, QHBoxLayout
from PySide6.QtWidgets import QWidget, QHeaderView, QGridLayout, QPushButton, QMenu, QCommandLinkButton
from PySide6.QtCore import Qt, QMimeData, QSize, QProcess, Signal, QObject, QUrl
from PySide6.QtGui import QContextMenuEvent, QDrag, QPixmap, QCursor, QAction,QIcon, QMovie, QDesktopServices
import os
import sys
...
class DraggableWidget(QWidget):
    ...
    def set_button_icon(self):
        ...
        if self.button.isChecked():
            image_path = "/home/rapa/xgen/saved.png"
        else:
            image_path = "/home/rapa/xgen/save.png"
...
class LibraryLoader(QWidget):
    def __init__(self):
        ...
        # self.clicked.connect(self.set_clip_favorites_in_tableWidget)

        # reset button
        self.ui_pushButton_reset = QPushButton(self)
        self.ui_pushButton_reset.setGeometry(1077, 20, 33,33)  # 버튼의 위치와 크기 설정
        self.ui_pushButton_reset.clicked.connect(self.reset_ui)


        # 이미지 경로 설정
        image_path = "/home/rapa/xgen/icon-reload.png"

        # QPixmap을 사용하여 이미지를 로드하고 QIcon으로 변환
        pixmap = QPixmap(image_path)
        
        # 버튼 크기에 맞게 이미지 크기 조정 (비율 유지)
        button_size = self.ui_pushButton_reset.size()

        scaled_pixmap = pixmap.scaled(button_size, Qt.KeepAspectRatioByExpanding, Qt.SmoothTransformation)

        # QIcon으로 변환 후 버튼 아이콘으로 설정
        icon = QIcon(scaled_pixmap)
        self.ui_pushButton_reset.setIcon(icon)
        icon_size = QSize(button_size.width() -10, button_size.height() - 10)
        self.ui_pushButton_reset.setIconSize(icon_size)  # 아이콘 크기 설정

        # 스타일시트 설정
        self.ui_pushButton_reset.setStyleSheet(
            """
            QPushButton {
                # padding: 0px; /* 버튼 내부 여백 제거 */
                margin: 0px; /* 버튼 외부 여백 제거 */
    ...
    def set_fav_items_in_tableWidget(self, mov_path, image_path):
        ...
        for index, base_name in enumerate(fav_clip_existed_image_list):
            ...
            draggable_widget_fav = DraggableWidget(file_path, image_file_path)
