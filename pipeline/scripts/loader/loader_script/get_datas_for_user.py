...
class OpenLoaderData():
    def __init__(self,project_name):
        self.connect_sg()
        project_id = self.read_data_from_login_json(project_name)
        asset_data = self.get_asset_datas(project_id)
        asset_names = [asset["asset_name"] for asset in asset_data]
        asset_ver_data = self.get_asset_versions_data(project_id, asset_names)
        project_ver_data = self.get_project_versions_data(project_id)
        self.save_to_json(asset_data, asset_ver_data, project_ver_data)
    ...
    def read_data_from_login_json(self,selected_project):
        """
        ...
        for asset in asset_data:
            ...
            for version in asset_ver_data:
                ...
                if isinstance(linked_asset, dict):
                    ...
                    # 만약 linked_asset이 문자열이라면, 단순히 문자열을 사용할 수 있음
